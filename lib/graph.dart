...
void main() async {
  ...
  while (true) {
    // Keep the app running to observe changes
    await Future.delayed(const Duration(seconds: 1));
    if (graph.vertices.length > 5) continue;
    if (faker.randomGenerator.boolean()) {
      final from = City(faker.address.city());
      final to = City(faker.address.city());
      final relation = faker.lorem.word();
      graph.addRelation(from, to, relation);
    } else {
      final entity = City(faker.address.city());
